...
async function calculateDetailedReviewerMetrics(prFiles, prAuthor) {
  ...
  const finalMetrics = Array.from(contributorMetrics.values()).map(metrics => ({
    ...
    learns: filePaths.length, // They would learn about all PR files
...
| Developer | Knows | Learns | L-Commits | L-Reviews | G-Commits | G-Reviews | A-Months |
|-----------|-------|---------|-----------|-----------|-----------|-----------|----------|
`;
    
    reviewerMetrics.forEach(metrics => {
      comment += `| @${metrics.login} | ${metrics.knows} | ${metrics.learns} | ${metrics.lCommits} | ${metrics.lReviews} | ${metrics.gCommits} | ${metrics.gReviews} | ${metrics.aMonths} |\n`;
    });
    
    comment += `\n**Legend:**
- **Knows**: Files in this PR the candidate has worked on before
- **Learns**: Files in this PR the candidate would learn about (always = total PR files)
- **L-Commits**: Local commits on known files
- **L-Reviews**: Local reviews on known files  
...
`;
  ...
  comment += `\n---
*This analysis is based on historical commit and review data. The final decision on reviewers should consider team availability, workload, and project requirements.*
